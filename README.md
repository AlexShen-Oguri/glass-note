...
网站已上线，Windows x64 安装包以预发行测试版提供。下载 ZIP 后解压，运行其中的安装程序。安装包尚未签名，干净机器安装、升级与卸载验收仍待完成；升级前请导出备份。iOS 共用代码保留，App Store 发布暂缓。
...
源码、网站和 Windows 0.1.8 预发行包已公开。正式稳定版尚未发布。安全审查继续进行，测试或秘密扫描通过不代表全面安全认证。干净 Windows 安装、签名和实体手机验收仍待完成；已知范围与校验文件见 [发布说明](https://github.com/AlexShen-Oguri/glass-note/releases/tag/v0.1.8)。
