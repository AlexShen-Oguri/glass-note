...
function canonical(value: unknown, seen: Set<object>): string {
  ...
  if (typeof value === 'number') {
    if (!Number.isFinite(value)) throw new TypeError('semantic fingerprint only accepts finite JSON numbers');
    return JSON.stringify(Object.is(value, -0) ? 0 : value);
  }
  if (Array.isArray(value)) {
    if (seen.has(value)) throw new TypeError('semantic fingerprint does not accept cyclic values');
    seen.add(value);
    const result = `[${value.map(item => item === undefined ? 'null' : canonical(item, seen)).join(',')}]`;
    seen.delete(value);
    return result;
  }
  ...
  throw new TypeError('semantic fingerprint only accepts JSON values');
}
